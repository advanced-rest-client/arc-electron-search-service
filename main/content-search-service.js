...
class ContentSearchService {
  ...
  /**
   * Sets application startup options.
   * This options are used to build path to the theme file.
   * It is optional to set this property.
   * @type {Object}
   * @param {Object} value
   */
  static set startupOptions(value) {
    startupOptions = value;
  }
  /**
   * An instance of preferences manager to read current application
   * preferences to determine sources location in the app.
   * @type {PreferencesManager}
   * @param {PreferencesManager} value
   */
  static set prefsManager(value) {
    prefsManager = value;
  }
  /**
  ...
  open() {
    if (!prefsManager) {
      log.error('[main] Preferences manager not set in search service');
      return;
    }
    const sm = new SourcesManager(prefsManager, startupOptions);
    sm.getAppConfig()
    .then((cnf) => {
      let win = this._getWindow();
      this._loadUI(win, cnf);
   ...
   * @param {Object} cnf Application sources config
   */
  _loadUI(win, cnf) {
    const dest = path.join(__dirname, '..', 'renderer', 'search-bar.html');
    let full = url.format({
      pathname: dest,
      protocol: 'file:',
      slashes: true,
